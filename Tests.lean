...
namespace TestFrontmatter

open LeanAgent.CodingAgent.Utils.Frontmatter

def testCodingAgentFrontmatterParse : IO Unit := do
  let input := "---\nname: skill-name\ndescription: 'A desc'\nfoo-bar: value\n---\n\nBody text\n"
  let (pairs, body) := parseSimpleFrontmatter input
  assertTrue (pairs.any (fun (k, v) => k == "name" && v == "skill-name")) "parse: name"
  assertTrue (pairs.any (fun (k, v) => k == "description" && v == "A desc")) "parse: description"
  assertTrue (pairs.any (fun (k, v) => k == "foo-bar" && v == "value")) "parse: foo-bar"
  assertTrue (body.trimAscii.toString == "Body text") "parse: body"

def testCodingAgentFrontmatterStrip : IO Unit := do
  let input := "---\nkey: value\n---\n\nBody\n"
  let body := stripFrontmatter input
  assertTrue (body.trimAscii.toString == "Body") "strip: body"

  let noFm := "\n  No frontmatter body  \n"
  let body2 := stripFrontmatter noFm
  assertTrue (body2 == noFm) "strip: no frontmatter"

end TestFrontmatter

namespace TestMime

open LeanAgent.CodingAgent.Utils.Mime

def jpegMagic : ByteArray := ByteArray.mk #[0xff, 0xd8, 0xff, 0xe0, 0x00, 0x10]
def pngMagic : ByteArray := ByteArray.mk #[0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a, 0x00, 0x00]
def gifMagic : ByteArray := ByteArray.mk #[0x47, 0x49, 0x46, 0x38, 0x39, 0x61]
def webpMagic : ByteArray := ByteArray.mk #[0x52, 0x49, 0x46, 0x46, 0x00, 0x00, 0x00, 0x00, 0x57, 0x45, 0x42, 0x50]

def testCodingAgentMimeJpeg : IO Unit := do
  let result := detectSupportedImageMimeType jpegMagic
  assertTrue (result == some "image/jpeg") "jpeg: detected"

def testCodingAgentMimePng : IO Unit := do
  let result := detectSupportedImageMimeType pngMagic
  assertTrue (result == some "image/png") "png: detected"

def testCodingAgentMimeGif : IO Unit := do
  let result := detectSupportedImageMimeType gifMagic
  assertTrue (result == some "image/gif") "gif: detected"

def testCodingAgentMimeWebp : IO Unit := do
  let result := detectSupportedImageMimeType webpMagic
...
def main : IO UInt32 := do
  try
    ...
    TestFrontmatter.testCodingAgentFrontmatterParse
    TestFrontmatter.testCodingAgentFrontmatterStrip
    TestMime.testCodingAgentMimeJpeg
