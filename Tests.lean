...
open LeanAgent.CodingAgent.ModelRegistry
open LeanAgent.CodingAgent.AuthStorage
open LeanAgent.Models

/-- Set an environment variable (libuv FFI; Lean 4.31 has no `IO.setEnv`). -/
@[extern "lean_uv_os_setenv"]
opaque setEnv (name : String) (value : String) : IO Unit

/-- Unset an environment variable. -/
@[extern "lean_uv_os_unsetenv"]
opaque unsetEnv (name : String) : IO Unit

/-- Temp directory root for model-registry tests. -/
def tmpRoot : IO System.FilePath := do
  let cwd ← IO.currentDir
  pure (cwd / ".lake" / "model-registry-test")

/-- Wipe+create a fresh per-test subdirectory and run `action` with it. -/
def withFreshDir (name : String) (action : System.FilePath → IO α) : IO α := do
  let root ← tmpRoot
  let dir := root / name
...
/-- `AuthStorage` opened at `<dir>/auth.json` (empty to start). -/
def openAuth (dir : System.FilePath) : IO AuthStorage :=
  LeanAgent.CodingAgent.AuthStorage.AuthStorage.openStorage (some (dir / "auth.json"))

/-- Write a `{ "providers": { ... } }` JSON doc to `<dir>/models.json`. -/
def writeModelsJson (dir : System.FilePath) (providers : Array (String × Lean.Json)) :
    IO Unit := do
  let obj := LeanAgent.Json.obj [("providers", LeanAgent.Json.obj providers.toList)]
  IO.FS.writeFile (dir / "models.json") obj.pretty

/-- Open a registry backed by `<dir>/models.json` + the supplied auth. -/
def openRegistry (dir : System.FilePath) (auth : AuthStorage) : IO ModelRegistry := do
  LeanAgent.CodingAgent.ModelRegistry.ModelRegistry.create
    (AuthStorageLike.fromAuthStorage auth) defaultCatalog (some (dir / "models.json"))

/-- Filter a model array to one provider. -/
def forProvider (models : Array ModelInfo) (provider : String) : Array ModelInfo :=
  models.filter (fun m => m.provider == provider)

/-- Find a model by provider+id. -/
def findIn (models : Array ModelInfo) (provider id : String) : Option ModelInfo :=
  models.find? (fun m => m.provider == provider && m.id == id)

/-- Build a minimal provider-config JSON with `baseUrl`/`apiKey`/`api`/`models`. -/
def providerConfigJson (baseUrl apiKey api : String) (modelIds : Array String) : Lean.Json :=
  let models := modelIds.map fun id =>
    LeanAgent.Json.obj
      [ ("id", LeanAgent.Json.str id)
      , ("name", LeanAgent.Json.str id)
      , ("reasoning", LeanAgent.Json.bool false)
      , ("input", LeanAgent.Json.arr #[LeanAgent.Json.str "text"])
      , ("cost", LeanAgent.Json.obj
          [ ("input", LeanAgent.Json.nat 0), ("output", LeanAgent.Json.nat 0)
          , ("cacheRead", LeanAgent.Json.nat 0), ("cacheWrite", LeanAgent.Json.nat 0)])
      , ("contextWindow", LeanAgent.Json.nat 100000)
      , ("maxTokens", LeanAgent.Json.nat 8000)
      ]
  LeanAgent.Json.obj
    [ ("baseUrl", LeanAgent.Json.str baseUrl)
    , ("apiKey", LeanAgent.Json.str apiKey)
    , ("api", LeanAgent.Json.str api)
    , ("models", LeanAgent.Json.arr models)
    ]

/-- Build a baseUrl-only override config JSON. -/
def overrideBaseUrl (baseUrl : String) : Lean.Json :=
  LeanAgent.Json.obj [("baseUrl", LeanAgent.Json.str baseUrl)]

-- ============================================================================
-- baseUrl override (no custom models)
-- ============================================================================

def testBaseUrlOverrideKeepsBuiltInModels : IO Unit := do
  withFreshDir "baseUrl-keeps" fun dir => do
    writeModelsJson dir #[("anthropic", overrideBaseUrl "https://my-proxy.example.com/v1")]
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    let all ← registry.getAll
    let anthropic := forProvider all "anthropic"
    assertTrue (anthropic.size > 1) "baseUrl override keeps multiple built-in models"
    assertTrue (anthropic.any (fun m => m.id.contains "claude")) "built-in claude models present"

def testBaseUrlOverrideChangesUrl : IO Unit := do
  withFreshDir "baseUrl-changes" fun dir => do
    writeModelsJson dir #[("anthropic", overrideBaseUrl "https://my-proxy.example.com/v1")]
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    let all ← registry.getAll
    assertTrue ((forProvider all "anthropic").all (fun m => m.baseUrl == "https://my-proxy.example.com/v1"))
      "all anthropic models get the new baseUrl"

def testHeadersOnlyOverrideResolvesAtRequestTime : IO Unit := do
  withFreshDir "headers-only" fun dir => do
    let raw := LeanAgent.Json.obj
      [("headers", LeanAgent.Json.obj [("X-Custom-Header", LeanAgent.Json.str "custom-value")])]
    writeModelsJson dir #[("anthropic", raw)]
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    let err ← registry.getError
    assertTrue (err.isNone) "headers-only override is valid"
    let all ← registry.getAll
    let anthropic := forProvider all "anthropic"
    assertTrue (!anthropic.isEmpty) "anthropic models present"
    match anthropic[0]? with
    | some m =>
        match ← registry.getApiKeyAndHeaders m with
        | .ok _ (some hs) =>
            assertTrue (hs.any (fun p => p.1 == "X-Custom-Header" && p.2 == "custom-value"))
              "header resolves at request time"
        | _ => fail "expected ok auth with headers"
    | none => fail "no anthropic models"

def testBaseUrlOverrideDoesNotAffectOtherProviders : IO Unit := do
  withFreshDir "baseUrl-isolated" fun dir => do
    writeModelsJson dir #[("anthropic", overrideBaseUrl "https://my-proxy.example.com/v1")]
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    let all ← registry.getAll
    let google := forProvider all "google"
    assertTrue (google.size > 0) "google models present"
    match google[0]? with
    | some m => assertTrue (m.baseUrl != "https://my-proxy.example.com/v1") "google baseUrl unchanged"
    | none => fail "no google models"

def testMixBaseUrlOverrideAndCustomModels : IO Unit := do
  withFreshDir "mix-override" fun dir => do
    writeModelsJson dir
      #[ ("anthropic", overrideBaseUrl "https://anthropic-proxy.example.com/v1")
       , ("google", providerConfigJson "https://google-proxy.example.com/v1" "test-key"
            "google-generative-ai" #["gemini-custom"])
       ]
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    let all ← registry.getAll
    let anthropic := forProvider all "anthropic"
    assertTrue (anthropic.size > 1) "anthropic still has many models"
    assertTrue (anthropic[0]!.baseUrl == "https://anthropic-proxy.example.com/v1") "anthropic baseUrl overridden"
    let google := forProvider all "google"
    assertTrue (google.size > 1) "google has built-ins + custom"
    assertTrue (google.any (fun m => m.id == "gemini-custom")) "google custom model merged"

def testRefreshPicksUpBaseUrlChanges : IO Unit := do
  withFreshDir "refresh-baseurl" fun dir => do
    writeModelsJson dir #[("anthropic", overrideBaseUrl "https://first-proxy.example.com/v1")]
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    let all1 ← registry.getAll
    assertTrue ((forProvider all1 "anthropic")[0]!.baseUrl == "https://first-proxy.example.com/v1") "first baseUrl"
    writeModelsJson dir #[("anthropic", overrideBaseUrl "https://second-proxy.example.com/v1")]
    registry.refresh
    let all2 ← registry.getAll
    assertTrue ((forProvider all2 "anthropic")[0]!.baseUrl == "https://second-proxy.example.com/v1")
      "refresh picks up second baseUrl"

-- ============================================================================
-- custom models merge behavior
-- ============================================================================

def testBuiltInProviderCustomModelInheritsApiAndBaseUrl : IO Unit := do
  withFreshDir "inherit-fields" fun dir => do
    let provider := LeanAgent.Json.obj
      [ ("models", LeanAgent.Json.arr
          #[ LeanAgent.Json.obj
               [ ("id", LeanAgent.Json.str "fake-provider/fake-model")
               , ("name", LeanAgent.Json.str "Fake model")
               , ("reasoning", LeanAgent.Json.bool true)
               , ("input", LeanAgent.Json.arr #[LeanAgent.Json.str "text"])
               ]
           ])
      ]
    writeModelsJson dir #[("openrouter", provider)]
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    let err ← registry.getError
    assertTrue (err.isNone) "built-in provider custom model inherits fields"
    match ← registry.find "openrouter" "fake-provider/fake-model" with
    | some m =>
        assertTrue (m.api == "openai-completions") "inherits api"
        assertTrue (m.baseUrl == "https://openrouter.ai/api/v1") "inherits baseUrl"
    | none => fail "custom model not found"

def testNonBuiltInProviderRequiresBaseUrl : IO Unit := do
  withFreshDir "requires-baseurl" fun dir => do
    let provider := LeanAgent.Json.obj
      [ ("apiKey", LeanAgent.Json.str "test-key")
      , ("models", LeanAgent.Json.arr
          #[ LeanAgent.Json.obj
               [ ("id", LeanAgent.Json.str "my-model")
               , ("api", LeanAgent.Json.str "openai-completions")
               , ("reasoning", LeanAgent.Json.bool false)
               , ("input", LeanAgent.Json.arr #[LeanAgent.Json.str "text"])
               ]
           ])
      ]
    writeModelsJson dir #[("my-custom-provider", provider)]
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    match ← registry.getError with
    | some e => assertTrue (e.contains "baseUrl") "error mentions baseUrl"
    | none => fail "expected baseUrl-required error"

def testCustomModelSameIdReplacesBuiltIn : IO Unit := do
  withFreshDir "same-id-replaces" fun dir => do
    writeModelsJson dir
      #[ ("openrouter", providerConfigJson "https://my-proxy.example.com/v1" "test-key"
            "openai-completions" #["anthropic/claude-sonnet-4"])
       ]
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    let all ← registry.getAll
    let sonnet := all.filter (fun m => m.provider == "openrouter" && m.id == "anthropic/claude-sonnet-4")
    assertTrue (sonnet.size == 1) "exactly one sonnet after replace"
    assertTrue (sonnet[0]!.baseUrl == "https://my-proxy.example.com/v1") "replaced model has new baseUrl"

def testProviderCompatAppliesToCustomModels : IO Unit := do
  withFreshDir "compat-custom" fun dir => do
    let provider := LeanAgent.Json.obj
      [ ("baseUrl", LeanAgent.Json.str "https://example.com/v1")
      , ("apiKey", LeanAgent.Json.str "DEMO_KEY")
      , ("api", LeanAgent.Json.str "openai-completions")
      , ("compat", LeanAgent.Json.obj
          [ ("supportsUsageInStreaming", LeanAgent.Json.bool false)
          , ("maxTokensField", LeanAgent.Json.str "max_tokens")
          ])
      , ("models", LeanAgent.Json.arr
          #[ LeanAgent.Json.obj
               [ ("id", LeanAgent.Json.str "demo-model")
               , ("reasoning", LeanAgent.Json.bool false)
               , ("input", LeanAgent.Json.arr #[LeanAgent.Json.str "text"])
               , ("cost", LeanAgent.Json.obj
                   [ ("input", LeanAgent.Json.nat 0), ("output", LeanAgent.Json.nat 0)
                   , ("cacheRead", LeanAgent.Json.nat 0), ("cacheWrite", LeanAgent.Json.nat 0)])
               , ("contextWindow", LeanAgent.Json.nat 1000)
               , ("maxTokens", LeanAgent.Json.nat 100)
               ]
           ])
      ]
    writeModelsJson dir #[("demo", provider)]
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    match ← registry.find "demo" "demo-model" with
    | some m =>
        assertTrue (m.compat.supportsUsageInStreaming == false) "provider compat (usage)"
        assertTrue (m.compat.maxTokensField == "max_tokens") "provider compat (field)"
    | none => fail "demo-model missing"

def testModelCompatOverridesProviderCompat : IO Unit := do
  withFreshDir "compat-model-override" fun dir => do
    let modelCompat := LeanAgent.Json.obj
      [ ("supportsUsageInStreaming", LeanAgent.Json.bool true)
      , ("maxTokensField", LeanAgent.Json.str "max_completion_tokens")
      ]
    let model := LeanAgent.Json.obj
      [ ("id", LeanAgent.Json.str "demo-model")
      , ("reasoning", LeanAgent.Json.bool false)
      , ("input", LeanAgent.Json.arr #[LeanAgent.Json.str "text"])
      , ("cost", LeanAgent.Json.obj
    ...
    let provider := LeanAgent.Json.obj
      ...
      , ("models", LeanAgent.Json.arr #[model])
      ]
    writeModelsJson dir #[("demo", provider)]
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    match ← registry.find "demo" "demo-model" with
    | some m =>
        assertTrue (m.compat.supportsUsageInStreaming == true) "model compat overrides provider (usage)"
        assertTrue (m.compat.maxTokensField == "max_completion_tokens") "model compat overrides provider (field)"
    | none => fail "demo-model missing"

def testModelBaseUrlOverridesProviderBaseUrl : IO Unit := do
  withFreshDir "model-baseurl" fun dir => do
    let cost0 := LeanAgent.Json.obj
      [ ("input", LeanAgent.Json.nat 0), ("output", LeanAgent.Json.nat 0)
      , ("cacheRead", LeanAgent.Json.nat 0), ("cacheWrite", LeanAgent.Json.nat 0)]
    let m1 := LeanAgent.Json.obj
      [ ("id", LeanAgent.Json.str "minimax-m2.5")
      , ("api", LeanAgent.Json.str "anthropic-messages")
      , ("baseUrl", LeanAgent.Json.str "https://opencode.ai/zen/go")
      , ("reasoning", LeanAgent.Json.bool true)
      , ("input", LeanAgent.Json.arr #[LeanAgent.Json.str "text"])
      , ("cost", cost0)
      , ("contextWindow", LeanAgent.Json.nat 204800)
      , ("maxTokens", LeanAgent.Json.nat 131072)
      ]
    let m2 := LeanAgent.Json.obj
      [ ("id", LeanAgent.Json.str "glm-5")
      , ("api", LeanAgent.Json.str "openai-completions")
      , ("reasoning", LeanAgent.Json.bool true)
      , ("input", LeanAgent.Json.arr #[LeanAgent.Json.str "text"])
      , ("cost", cost0)
    ...
    let provider := LeanAgent.Json.obj
      [ ("baseUrl", LeanAgent.Json.str "https://opencode.ai/zen/go/v1")
      , ("apiKey", LeanAgent.Json.str "TEST_KEY")
      , ("models", LeanAgent.Json.arr #[m1, m2])
      ]
    writeModelsJson dir #[("opencode-go", provider)]
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    match ← registry.find "opencode-go" "minimax-m2.5", ← registry.find "opencode-go" "glm-5" with
    | some m25, some glm5 =>
        assertTrue (m25.baseUrl == "https://opencode.ai/zen/go") "model baseUrl overrides provider"
        assertTrue (glm5.baseUrl == "https://opencode.ai/zen/go/v1") "sibling inherits provider baseUrl"
    | _, _ => fail "models missing"

def testRemovingCustomModelsKeepsBuiltIn : IO Unit := do
  withFreshDir "remove-keeps-builtin" fun dir => do
    writeModelsJson dir
      #[ ("anthropic", providerConfigJson "https://proxy.example.com/v1" "test-key"
            "anthropic-messages" #["claude-custom"])
       ]
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    let all1 ← registry.getAll
    assertTrue ((forProvider all1 "anthropic").any (fun m => m.id == "claude-custom")) "custom present initially"
    writeModelsJson dir #[]
    registry.refresh
    let all2 ← registry.getAll
    let anthropic := forProvider all2 "anthropic"
    assertTrue (!anthropic.any (fun m => m.id == "claude-custom")) "custom gone after refresh"
    assertTrue (anthropic.any (fun m => m.id.contains "claude")) "built-in claude models remain"

-- ============================================================================
-- modelOverrides (per-model customization)
-- ============================================================================

def testModelOverrideAppliesToSingleBuiltIn : IO Unit := do
  withFreshDir "override-single" fun dir => do
    let mo := LeanAgent.Json.obj
      [("anthropic/claude-sonnet-4", LeanAgent.Json.obj [("name", LeanAgent.Json.str "Custom Sonnet Name")])]
    writeModelsJson dir #[("openrouter", LeanAgent.Json.obj [("modelOverrides", mo)])]
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    let all ← registry.getAll
    match findIn all "openrouter" "anthropic/claude-sonnet-4" with
    | some m => assertTrue (m.name == "Custom Sonnet Name") "sonnet name overridden"
    | none => fail "sonnet not found"
    match findIn all "openrouter" "anthropic/claude-opus-4" with
    | some o => assertTrue (o.name != "Custom Sonnet Name") "opus untouched"
    | none => pure ()

def testModelOverrideCompatOpenRouterRouting : IO Unit := do
  withFreshDir "override-routing" fun dir => do
    let routing := LeanAgent.Json.obj [("only", LeanAgent.Json.arr #[LeanAgent.Json.str "amazon-bedrock"])]
    let mo := LeanAgent.Json.obj
      [ ( "anthropic/claude-sonnet-4"
        , LeanAgent.Json.obj [("compat", LeanAgent.Json.obj [("openRouterRouting", routing)])])
      ]
    writeModelsJson dir #[("openrouter", LeanAgent.Json.obj [("modelOverrides", mo)])]
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    ...
    match findIn all "openrouter" "anthropic/claude-sonnet-4" with
    | some m =>
        assertTrue (m.compat.openRouterRouting.isSome) "openRouterRouting set"
        match m.compat.openRouterRouting with
        | some r =>
            match r.getObjVal? "only" with
            | .ok arr =>
                match arr.getArrElem? 0 with
                | .ok (.str s) => assertTrue (s == "amazon-bedrock") "routing only[0]"
                | _ => fail "routing only[0] not string"
            | _ => fail "routing only missing"
        | none => fail "openRouterRouting missing"
    | none => fail "sonnet not found"

def testModelOverrideForNonExistentIdIgnored : IO Unit := do
  withFreshDir "override-nonexistent" fun dir => do
    let mo := LeanAgent.Json.obj
      [ ( "nonexistent/model-id"
        , LeanAgent.Json.obj [("name", LeanAgent.Json.str "Should not appear")])
      ]
    writeModelsJson dir #[("openrouter", LeanAgent.Json.obj [("modelOverrides", mo)])]
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    let all ← registry.getAll
    assertTrue (findIn all "openrouter" "nonexistent/model-id" |>.isNone) "nonexistent override creates no model"
    let err ← registry.getError
    assertTrue (err.isNone) "no load error for ignored override"

def testModelOverridePartialCost : IO Unit := do
  withFreshDir "override-cost" fun dir => do
    let mo := LeanAgent.Json.obj
      [ ( "anthropic/claude-sonnet-4"
        , LeanAgent.Json.obj [("cost", LeanAgent.Json.obj [("input", LeanAgent.Json.nat 99)])])
      ]
    writeModelsJson dir #[("openrouter", LeanAgent.Json.obj [("modelOverrides", mo)])]
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    ...
    match findIn all "openrouter" "anthropic/claude-sonnet-4" with
    | some m =>
        assertTrue (m.cost.input == 99.0) "input cost overridden"
        assertTrue (m.cost.output > 0.0) "output cost preserved from built-in"
    | none => fail "sonnet not found"

def testModelOverrideRequestTimeHeaders : IO Unit := do
  withFreshDir "override-headers" fun dir => do
    let mo := LeanAgent.Json.obj
      [ ( "anthropic/claude-sonnet-4"
        , LeanAgent.Json.obj
            [("headers", LeanAgent.Json.obj [("X-Custom-Model-Header", LeanAgent.Json.str "value")])])
      ]
    writeModelsJson dir #[("openrouter", LeanAgent.Json.obj [("modelOverrides", mo)])]
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    ...
    | some m =>
        match ← registry.getApiKeyAndHeaders m with
        | .ok _ (some hs) =>
            assertTrue (hs.any (fun p => p.1 == "X-Custom-Model-Header" && p.2 == "value"))
              "model override header resolves at request time"
        | _ => fail "expected ok with headers"
    | none => fail "sonnet not found"

def testRemovingModelOverrideRestoresBuiltIn : IO Unit := do
  withFreshDir "override-restore" fun dir => do
    let mo := LeanAgent.Json.obj
      [ ( "anthropic/claude-sonnet-4"
        , LeanAgent.Json.obj [("name", LeanAgent.Json.str "Custom Name")])
      ]
    writeModelsJson dir #[("openrouter", LeanAgent.Json.obj [("modelOverrides", mo)])]
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    let all1 ← registry.getAll
    let customName := findIn all1 "openrouter" "anthropic/claude-sonnet-4" |>.map (·.name)
    assertTrue (customName == some "Custom Name") "custom name applied"
    writeModelsJson dir #[]
    registry.refresh
    let all2 ← registry.getAll
    let restoredName := findIn all2 "openrouter" "anthropic/claude-sonnet-4" |>.map (·.name)
    assertTrue (restoredName != some "Custom Name") "name restored after removing override"

-- ============================================================================
-- dynamic provider lifecycle
-- ============================================================================

def testGetProviderDisplayNameFallbacks : IO Unit := do
  withFreshDir "display-name" fun dir => do
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    let n1 ← registry.getProviderDisplayName "openai"
    assertTrue (n1 == "OpenAI") "built-in display name"
    let n2 ← registry.getProviderDisplayName "unknown-provider"
    assertTrue (n2 == "unknown-provider") "unknown falls back to id"
    let cfg : ProviderConfig :=
      { name := some "Named Provider"
        baseUrl := some "https://provider.test/v1"
        apiKey := some "test-key"
        api := some "openai-completions"
        models :=
          #[ { id := "demo-model"
               name := some "demo-model"
               input := #["text"]
               cost := some (0.0, 0.0, 0.0, 0.0)
               contextWindow := some 128000
               maxTokens := some 4096 } ] }
    registry.registerProvider "named-provider" cfg
    let n3 ← registry.getProviderDisplayName "named-provider"
    assertTrue (n3 == "Named Provider") "registered name wins"

def testStoredEnvPropagatesToRequestAuth : IO Unit := do
  withFreshDir "stored-env" fun dir => do
    let auth ← openAuth dir
    let env := envMapFromList
      [ ("CLOUDFLARE_API_KEY", "stored-cf-token")
      , ("CLOUDFLARE_ACCOUNT_ID", "stored-account")
      ]
    auth.set "cloudflare-ai-gateway" (.apiKey "$CLOUDFLARE_API_KEY" env)
    let provider := LeanAgent.Json.obj
      [("headers", LeanAgent.Json.obj [("x-account", LeanAgent.Json.str "$CLOUDFLARE_ACCOUNT_ID")])]
    writeModelsJson dir #[("cloudflare-ai-gateway", provider)]
    let registry ← openRegistry dir auth
    let all ← registry.getAll
    match all.find? (fun m => m.provider == "cloudflare-ai-gateway") with
    | some m =>
        match ← registry.getApiKeyAndHeaders m with
        | .ok (some k) (some hs) (some envArr) =>
            assertTrue (k == "stored-cf-token") "apiKey resolved from stored env"
            assertTrue (hs.any (fun p => p.1 == "x-account" && p.2 == "stored-account"))
              "header resolved from stored env"
            assertTrue (envArr.any (fun p => p.1 == "CLOUDFLARE_API_KEY" && p.2 == "stored-cf-token"))
              "env map propagated"
        | _ => fail "expected ok auth with all fields"
    | none => fail "cloudflare-ai-gateway model missing"

def testRegisterProviderUppercaseLiteral : IO Unit := do
  withFreshDir "uppercase-literal" fun dir => do
    setEnv "CUSTOM_NAME" "env-CUSTOM_NAME"
    setEnv "BEARER" "env-BEARER"
    setEnv "MODEL_TOKEN" "env-MODEL_TOKEN"
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    let cfg : ProviderConfig :=
      { baseUrl := some "https://provider.test/v1"
        apiKey := some "CUSTOM_NAME"
        api := some "openai-completions"
        headers := some #[("Authorization", "BEARER")]
        models :=
          #[ { id := "demo-model"
               name := some "demo-model"
               input := #["text"]
               cost := some (0.0, 0.0, 0.0, 0.0)
               contextWindow := some 100000
               maxTokens := some 8000
               headers := some #[("x-model-token", "MODEL_TOKEN")] } ] }
    registry.registerProvider "literal-provider" cfg
    let key ← registry.getApiKeyForProvider "literal-provider"
    assertTrue (key == some "CUSTOM_NAME") "uppercase apiKey treated as literal"
    match ← registry.find "literal-provider" "demo-model" with
    | some m =>
        match ← registry.getApiKeyAndHeaders m with
        | .ok (some k) (some hs) _ =>
            assertTrue (k == "CUSTOM_NAME") "request apiKey literal"
            assertTrue (hs.any (fun p => p.1 == "Authorization" && p.2 == "BEARER"))
              "header literal"
            assertTrue (hs.any (fun p => p.1 == "x-model-token" && p.2 == "MODEL_TOKEN"))
              "model-level header literal"
        | _ => fail "expected ok auth"
    | none => fail "model missing"

def testFailedRegisterProviderKeepsExisting : IO Unit := do
  withFreshDir "failed-register" fun dir => do
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
      ...
      { baseUrl := some "https://provider.test/v1"
        ...
        models :=
          #[ { id := "demo-model"
               ...
               contextWindow := some 128000
               maxTokens := some 4096 } ] }
    registry.registerProvider "demo-provider" good
    assertTrue ((← registry.find "demo-provider" "demo-model").isSome) "first registration succeeds"
    -- Second registration with a model missing api AND no provider api → throws.
    let bad : ProviderConfig :=
      { baseUrl := some "https://provider.test/v2"
        apiKey := some "test-key"
        models :=
          #[ { id := "broken-model"
               name := some "Broken Model"
               input := #["text"]
               cost := some (0.0, 0.0, 0.0, 0.0)
               contextWindow := some 128000
               maxTokens := some 4096 } ] }
    let threw ← try
      registry.registerProvider "demo-provider" bad
      pure false
    catch _ => pure true
    assertTrue threw "invalid registration throws"
    assertTrue ((← registry.find "demo-provider" "demo-model").isSome)
      "existing model preserved after failed re-register"

def testDynamicBaseUrlOverridePersistsAcrossRefresh : IO Unit := do
  withFreshDir "dynamic-baseurl-persist" fun dir => do
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    registry.registerProvider "anthropic"
      { baseUrl := some "https://proxy.test/anthropic" }
    registry.refresh
    let all ← registry.getAll
    let anthropic := forProvider all "anthropic"
    assertTrue (anthropic.size > 1) "built-in models survive refresh"
    assertTrue (anthropic.all (fun m => m.baseUrl == "https://proxy.test/anthropic"))
      "dynamic baseUrl override persists"

def testDynamicModelsReplacementPersistsAcrossRefresh : IO Unit := do
  withFreshDir "dynamic-models-persist" fun dir => do
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    let cfg : ProviderConfig :=
      { baseUrl := some "https://custom.test/anthropic"
        apiKey := some "test-key"
        api := some "anthropic-messages"
        models :=
          #[ { id := "custom-claude"
               name := some "custom-claude"
               input := #["text"]
               cost := some (0.0, 0.0, 0.0, 0.0)
               contextWindow := some 128000
               maxTokens := some 4096 } ] }
    registry.registerProvider "anthropic" cfg
    registry.refresh
    let all ← registry.getAll
    let ids := (forProvider all "anthropic").map (·.id)
    assertTrue (ids == #["custom-claude"]) "models fully replaced after refresh"

def testDynamicHeadersOverridePersistsAcrossRefresh : IO Unit := do
  withFreshDir "dynamic-headers-persist" fun dir => do
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    let baseCfg : ProviderConfig :=
      { baseUrl := some "https://custom.test/v1"
        apiKey := some "test-key"
        api := some "openai-completions"
        models :=
          #[ { id := "custom-a"
           ...
           , { id := "custom-b"
               ...
               contextWindow := some 128000
               maxTokens := some 4096 } ] }
    registry.registerProvider "custom-provider" baseCfg
    registry.registerProvider "custom-provider"
      { headers := some #[("x-proxy", "enabled")] }
    registry.refresh
    let all ← registry.getAll
    let models := forProvider all "custom-provider"
    assertTrue (models.map (·.id) == #["custom-a", "custom-b"]) "custom models persist"
    assertTrue (models.all (fun m => m.baseUrl == "https://custom.test/v1")) "baseUrl preserved"
    match models[0]? with
    | some m =>
        match ← registry.getApiKeyAndHeaders m with
        | .ok _ (some hs) =>
            assertTrue (hs.any (fun p => p.1 == "x-proxy" && p.2 == "enabled"))
              "headers-only override persists"
        | _ => fail "expected ok with headers"
    | none => fail "no models"

-- ============================================================================
-- API key resolution (`!command`, `$VAR`, literals)
-- ============================================================================

def providerWithApiKey (apiKey : String) : Lean.Json :=
  LeanAgent.Json.obj
    [ ("baseUrl", LeanAgent.Json.str "https://example.com/v1")
    , ("apiKey", LeanAgent.Json.str apiKey)
    , ("api", LeanAgent.Json.str "anthropic-messages")
    , ("models", LeanAgent.Json.arr
        #[ LeanAgent.Json.obj
            [ ("id", LeanAgent.Json.str "test-model")
            , ("name", LeanAgent.Json.str "Test Model")
            , ("reasoning", LeanAgent.Json.bool false)
            , ("input", LeanAgent.Json.arr #[LeanAgent.Json.str "text"])
            , ("cost", LeanAgent.Json.obj
                [ ("input", LeanAgent.Json.nat 0), ("output", LeanAgent.Json.nat 0)
...
def testApiKeyCommandExecutes : IO Unit := do
  withFreshDir "api-key-command" fun dir => do
    writeModelsJson dir #[("custom-provider", providerWithApiKey "!echo test-api-key-from-command")]
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    let key ← registry.getApiKeyForProvider "custom-provider"
    assertTrue (key == some "test-api-key-from-command") "command apiKey executes and trims"

def testApiKeyCommandTrimsWhitespace : IO Unit := do
  withFreshDir "api-key-trim" fun dir => do
    writeModelsJson dir #[("custom-provider", providerWithApiKey "!printf '  spaced-key  '")]
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    let key ← registry.getApiKeyForProvider "custom-provider"
    assertTrue (key == some "spaced-key") "command output trimmed"

def testApiKeyCommandFailureReturnsNone : IO Unit := do
  withFreshDir "api-key-fail" fun dir => do
    writeModelsJson dir #[("custom-provider", providerWithApiKey "!exit 1")]
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    let key ← registry.getApiKeyForProvider "custom-provider"
    assertTrue (key.isNone) "failed command returns none"

def testApiKeyEnvVar : IO Unit := do
  withFreshDir "api-key-env" fun dir => do
    setEnv "TEST_MR_API_KEY_12345" "env-api-key-value"
    writeModelsJson dir #[("custom-provider", providerWithApiKey "$TEST_MR_API_KEY_12345")]
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    let key ← registry.getApiKeyForProvider "custom-provider"
    assertTrue (key == some "env-api-key-value") "$VAR resolves from env"

def testApiKeyBracedEnv : IO Unit := do
  withFreshDir "api-key-braced" fun dir => do
    setEnv "TEST_MR_BRACED_KEY_12345" "braced-env-value"
    writeModelsJson dir #[("custom-provider", providerWithApiKey "${TEST_MR_BRACED_KEY_12345}")]
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    let key ← registry.getApiKeyForProvider "custom-provider"
    assertTrue (key == some "braced-env-value") "${VAR} resolves"

def testApiKeyInterpolation : IO Unit := do
  withFreshDir "api-key-interp" fun dir => do
    setEnv "TEST_MR_PART_A" "left"
    setEnv "TEST_MR_PART_B" "right"
    writeModelsJson dir #[("custom-provider", providerWithApiKey "${TEST_MR_PART_A}_right")]
    -- Use a true interpolation that won't be confused with literals.
    writeModelsJson dir #[("custom-provider", providerWithApiKey "${TEST_MR_PART_A}_${TEST_MR_PART_B}")]
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    let key ← registry.getApiKeyForProvider "custom-provider"
    assertTrue (key == some "left_right") "interpolated env refs resolve"

def testApiKeyDollarDollarLiteral : IO Unit := do
  withFreshDir "api-key-dd" fun dir => do
    writeModelsJson dir #[("custom-provider", providerWithApiKey "$$TEST_MR_API_KEY_12345")]
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    let key ← registry.getApiKeyForProvider "custom-provider"
    assertTrue (key == some "$TEST_MR_API_KEY_12345") "$$ escapes a literal dollar"

def testApiKeyLiteralUsedDirectly : IO Unit := do
  withFreshDir "api-key-literal" fun dir => do
    writeModelsJson dir #[("custom-provider", providerWithApiKey "literal_api_key_value")]
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    let key ← registry.getApiKeyForProvider "custom-provider"
    assertTrue (key == some "literal_api_key_value") "literal apiKey used directly"

def testApiKeyCommandPerLookup : IO Unit := do
  withFreshDir "api-key-per-lookup" fun dir => do
    let counter := dir / "counter"
    IO.FS.writeFile counter "0"
    let cmd := s!"!sh -c 'count=$(cat \"{counter}\"); echo $$((count + 1)) > \"{counter}\"; echo key-value'"
    writeModelsJson dir #[("custom-provider", providerWithApiKey cmd)]
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    -- getApiKeyForProvider is uncached (Pi parity): three lookups, three executions.
    let _ ← registry.getApiKeyForProvider "custom-provider"
    let _ ← registry.getApiKeyForProvider "custom-provider"
    let _ ← registry.getApiKeyForProvider "custom-provider"
    let count ← IO.FS.readFile counter
    assertTrue (count.trimAscii.toString == "3") "command runs once per lookup (uncached)"

-- ============================================================================
-- auth status + getAvailable
-- ============================================================================

def testAuthStatusEnvVar : IO Unit := do
  withFreshDir "auth-status-env" fun dir => do
    setEnv "TEST_MR_STATUS_KEY_98765" "status-test-key"
    writeModelsJson dir #[("custom-provider", providerWithApiKey "$TEST_MR_STATUS_KEY_98765")]
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    let status ← registry.getProviderAuthStatus "custom-provider"
    assertTrue (status.configured) "env-backed apiKey configured"
    assertTrue (status.source == some "environment") "source is environment"
    assertTrue (status.label == some "TEST_MR_STATUS_KEY_98765") "label names the env var"

def testAuthStatusLiteralKey : IO Unit := do
  withFreshDir "auth-status-literal" fun dir => do
    writeModelsJson dir #[("custom-provider", providerWithApiKey "literal_api_key_value")]
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    let status ← registry.getProviderAuthStatus "custom-provider"
    assertTrue (status.configured) "literal configured"
    assertTrue (status.source == some "models_json_key") "source is models_json_key"

def testAuthStatusCommandNoExecution : IO Unit := do
  withFreshDir "auth-status-cmd" fun dir => do
    let counter := dir / "status-counter"
    IO.FS.writeFile counter "0"
    let cmd := s!"!sh -c 'echo 1 > \"{counter}\"; echo key-value'"
    writeModelsJson dir #[("custom-provider", providerWithApiKey cmd)]
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    let status ← registry.getProviderAuthStatus "custom-provider"
    assertTrue (status.source == some "models_json_command") "command source detected"
    let count ← IO.FS.readFile counter
    assertTrue (count.trimAscii.toString == "0") "getProviderAuthStatus does not execute the command"

def testMissingEnvKeyUnconfiguredAndFiltered : IO Unit := do
  withFreshDir "auth-status-missing" fun dir => do
    setEnv "TEST_MR_MISSING_KEY_98765" ""
    match (← IO.getEnv "TEST_MR_MISSING_KEY_98765") with
    | some _ => setEnv "TEST_MR_MISSING_KEY_98765" "" -- ensure set
    | none => pure ()
    -- Force unset by setting to empty won't work reliably across env semantics;
    -- use a var name we never set.
    writeModelsJson dir #[("custom-provider", providerWithApiKey "$TEST_MR_NEVER_SET_VAR_99999")]
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    let status ← registry.getProviderAuthStatus "custom-provider"
    assertTrue (!status.configured) "missing env → unconfigured"
    let available ← registry.getAvailable
    assertTrue (!available.any (fun m => m.provider == "custom-provider"))
      "getAvailable filters unconfigured provider"

def testGetAvailableDoesNotExecuteCommands : IO Unit := do
  withFreshDir "getavailable-noexec" fun dir => do
    let counter := dir / "counter"
    IO.FS.writeFile counter "0"
    let cmd := s!"!sh -c 'count=$(cat \"{counter}\"); echo $$((count + 1)) > \"{counter}\"; echo key-value'"
    writeModelsJson dir #[("custom-provider", providerWithApiKey cmd)]
    ...
    let available ← registry.getAvailable
    assertTrue (available.any (fun m => m.provider == "custom-provider"))
      "command-backed provider is available without execution"
    let count ← IO.FS.readFile counter
    assertTrue (count.trimAscii.toString == "0") "getAvailable did not execute the command"

def testGetApiKeyAndHeadersAuthHeaderPerRequest : IO Unit := do
  withFreshDir "authheader-per-request" fun dir => do
    let token := dir / "token"
    IO.FS.writeFile token "token-1"
    let cfg := LeanAgent.Json.obj
      [ ("baseUrl", LeanAgent.Json.str "https://example.com/v1")
      , ("apiKey", LeanAgent.Json.str s!"!sh -c 'cat \"{token}\"'")
      , ("api", LeanAgent.Json.str "anthropic-messages")
      , ("authHeader", LeanAgent.Json.bool true)
      , ("models", LeanAgent.Json.arr
          #[ LeanAgent.Json.obj
              [ ("id", LeanAgent.Json.str "test-model")
              , ("name", LeanAgent.Json.str "Test Model")
    ...
    writeModelsJson dir #[("custom-provider", cfg)]
    let auth ← openAuth dir
    let registry ← openRegistry dir auth
    match ← registry.find "custom-provider" "test-model" with
    | some m =>
        match ← registry.getApiKeyAndHeaders m with
        | .ok (some k) (some hs) _ =>
            assertTrue (k == "token-1") "apiKey resolved"
            assertTrue (hs.any (fun p => p.1 == "Authorization" && p.2 == "Bearer token-1"))
              "authHeader injects Bearer"
        | _ => fail "expected ok auth"
    | none => fail "model missing"

def testGetApiKeyAndHeadersErrorForFailedAuthHeader : IO Unit := do
  withFreshDir "authheader-fail" fun dir => do
    let cfg := LeanAgent.Json.obj
      [ ("baseUrl", LeanAgent.Json.str "https://example.com/v1")
      , ("apiKey", LeanAgent.Json.str "!exit 1")
      , ("api", LeanAgent.Json.str "anthropic-messages")
      , ("authHeader", LeanAgent.Json.bool true)
      , ("models", LeanAgent.Json.arr
          #[ LeanAgent.Json.obj
    ...
    | some m =>
        match ← registry.getApiKeyAndHeaders m with
        | .error _ => pure ()
        | _ => fail "expected error for failed authHeader resolution"
    | none => fail "model missing"

end TestModelRegistry

-- ============================================================================
-- Trust manager (Pi `packages/coding-agent/test/trust-manager.test.ts`)
-- ============================================================================

namespace TestTrustManager

open LeanAgent.CodingAgent.TrustManager

/-- Build an in-memory ProjectTrustStore for offline tests. -/
def mkStore (initial : Std.HashMap String (Option Bool) := {}) : IO ProjectTrustStore := do
  let tmp ← IO.FS.createTempDir
  let trustPath := tmp / "trust.json"
  -- Write initial data if any.
  if !initial.isEmpty then
    writeTrustFile trustPath initial
  ProjectTrustStore.create tmp

/-- Helper: get decision as string for assertions. -/
def decStr (d : ProjectTrustDecision) : String :=
  match d with
  | some true => "true"
  | some false => "false"
  | none => "null"

def testTrustStoreGetSet : IO Unit := do
  let store ← mkStore
  -- Initial: no decision.
  let d ← store.get "/home/user/project"
  assertTrue (d == none) "initial: no decision"
  -- Set trusted.
  store.set "/home/user/project" (some true)
  let d ← store.get "/home/user/project"
  assertTrue (d == some true) "set: trusted"
  -- Set not trusted.
  store.set "/home/user/project" (some false)
  let d ← store.get "/home/user/project"
  assertTrue (d == some false) "set: not trusted"
  -- Clear decision.
  store.set "/home/user/project" none
  let d ← store.get "/home/user/project"
  assertTrue (d == none) "set: cleared"

def testTrustStoreSetMany : IO Unit := do
  let store ← mkStore
  store.setMany #[
    { path := "/a", decision := some true },
    { path := "/a/b", decision := some false },
    { path := "/a/b/c", decision := none }
  ]
  assertTrue ((← store.get "/a") == some true) "setMany: /a trusted"
  assertTrue ((← store.get "/a/b") == some false) "setMany: /a/b not trusted"
  assertTrue ((← store.get "/a/b/c") == some false) "setMany: /a/b/c inherits /a/b false"

def testTrustStoreNearestAncestor : IO Unit := do
  let store ← mkStore
  -- Set trust on /a, check that /a/b inherits it.
  store.set "/a" (some true)
  let d ← store.get "/a/b/c"
  assertTrue (d == some true) "ancestor: /a/b/c inherits /a"
  -- Override /a/b with false, check that /a/b/c gets false.
  store.set "/a/b" (some false)
  let d ← store.get "/a/b/c"
  assertTrue (d == some false) "ancestor: /a/b/c gets /a/b override"
  -- /other is unaffected.
  let d ← store.get "/other"
  assertTrue (d == none) "ancestor: /other unaffected"

def testTrustOptions : IO Unit := do
  let opts ← getProjectTrustOptions "/home/user/project" false
  -- Without session-only: Trust, Trust parent, Do not trust (3 options).
  assertTrue (opts.size == 3) "options: 3 without session-only"
  assertTrue (opts[0]!.label == "Trust") "options: first is Trust"
  assertTrue (opts[0]!.trusted) "options: Trust is trusted"
  assertTrue (opts[2]!.label == "Do not trust") "options: last is Do not trust"
  assertTrue (!opts[2]!.trusted) "options: Do not trust is not trusted"
  -- With session-only: Trust, Trust parent, Trust (session), Do not trust, Do not trust (session).
  let opts ← getProjectTrustOptions "/home/user/project" true
  assertTrue (opts.size == 5) "options: 5 with session-only"
  -- Trust option has updates.
  assertTrue (opts[0]!.updates.size > 0) "options: Trust has updates"
  -- Session-only options have no updates.
  let sessionOpts := opts.filter (fun o => o.label.contains "this session only")
  for o in sessionOpts do
    assertTrue (o.updates.isEmpty) s!"options: session-only '{o.label}' has no updates"

def testHasTrustRequiringResources : IO Unit := do
  -- Create a temp dir without trust-requiring resources.
  let tmp ← IO.FS.createTempDir
  let noResources ← hasTrustRequiringProjectResources tmp.toString
  assertTrue (!noResources) "hasResources: empty dir returns false"
  -- Create .pi/settings.json → should return true.
  let piDir := tmp / ".pi"
  IO.FS.createDirAll piDir
  IO.FS.writeFile (piDir / "settings.json") "{}"
  let hasResources ← hasTrustRequiringProjectResources tmp.toString
  assertTrue hasResources "hasResources: .pi/settings.json returns true"

end TestTrustManager

-- ============================================================================
-- Project trust (Pi `packages/coding-agent/test/project-trust.test.ts`)
-- ============================================================================

namespace TestProjectTrust

open LeanAgent.CodingAgent.TrustManager
open LeanAgent.CodingAgent.ProjectTrust
...
/-- A ProjectTrustContext that returns the first option (simulating user selecting "Trust"). -/
def selectFirstContext : ProjectTrustContext :=
  { hasUI := true
    select := fun _ labels => pure (labels[0]?) }

/-- A ProjectTrustContext with no UI. -/
def noUIContext : ProjectTrustContext :=
  { hasUI := false }

def testResolveProjectTrustedOverride : IO Unit := do
  let store ← mkStore
  let opts : ResolveProjectTrustedOptions :=
    { cwd := "/tmp/test"
      trustStore := store
      trustOverride := some true
      projectTrustContext := noUIContext }
  -- trustOverride = true → trusted regardless of resources.
  let trusted ← resolveProjectTrusted opts
  assertTrue trusted "override: true → trusted"
  -- trustOverride = false → not trusted.
  let trusted ← resolveProjectTrusted { opts with trustOverride := some false }
  assertTrue (!trusted) "override: false → not trusted"

def testResolveProjectTrustedNoResources : IO Unit := do
  let store ← mkStore
  -- Create a temp dir without trust-requiring resources.
  let tmp ← IO.FS.createTempDir
  let opts : ResolveProjectTrustedOptions :=
    { cwd := tmp.toString
      ...
      projectTrustContext := noUIContext }
  let trusted ← resolveProjectTrusted opts
  assertTrue trusted "noResources: empty dir → trusted"

def testResolveProjectTrustedSavedDecision : IO Unit := do
  let store ← mkStore
  -- Create a temp dir with .pi/settings.json to trigger trust check.
  let tmp ← IO.FS.createTempDir
  let piDir := tmp / ".pi"
  IO.FS.createDirAll piDir
  IO.FS.writeFile (piDir / "settings.json") "{}"
  -- Pre-save a trust decision for the temp dir.
  store.set tmp.toString (some true)
  let opts : ResolveProjectTrustedOptions :=
    { cwd := tmp.toString
      trustStore := store
      projectTrustContext := noUIContext }
  -- Should use saved decision.
  let trusted ← resolveProjectTrusted opts
  assertTrue trusted "saved: uses saved true"
  -- Override with false.
  store.set tmp.toString (some false)
  let trusted ← resolveProjectTrusted opts
  assertTrue (!trusted) "saved: uses saved false"

def testResolveProjectTrustedDefaultAlways : IO Unit := do
  let store ← mkStore
  -- Create a temp dir with .pi/settings.json to trigger trust check.
  let tmp ← IO.FS.createTempDir
  let piDir := tmp / ".pi"
  ...
  let opts : ResolveProjectTrustedOptions :=
    { cwd := tmp.toString
      ...
      projectTrustContext := noUIContext }
  let trusted ← resolveProjectTrusted opts
  assertTrue trusted "defaultAlways: → trusted"

def testResolveProjectTrustedDefaultNever : IO Unit := do
  let store ← mkStore
  let tmp ← IO.FS.createTempDir
  let piDir := tmp / ".pi"
  IO.FS.createDirAll piDir
  ...
  let opts : ResolveProjectTrustedOptions :=
    { cwd := tmp.toString
      ...
      projectTrustContext := noUIContext }
  let trusted ← resolveProjectTrusted opts
  assertTrue (!trusted) "defaultNever: → not trusted"

def testResolveProjectTrustedAskNoUI : IO Unit := do
  let store ← mkStore
  let tmp ← IO.FS.createTempDir
  let piDir := tmp / ".pi"
  IO.FS.createDirAll piDir
  ...
  let opts : ResolveProjectTrustedOptions :=
    { cwd := tmp.toString
      ...
      projectTrustContext := noUIContext }
  -- ask + no UI → not trusted.
  let trusted ← resolveProjectTrusted opts
  assertTrue (!trusted) "askNoUI: → not trusted"

end TestProjectTrust
...
def main : IO UInt32 := do
  try
    ...
    TestModelRegistry.testBaseUrlOverrideKeepsBuiltInModels
    TestModelRegistry.testBaseUrlOverrideChangesUrl
    TestModelRegistry.testHeadersOnlyOverrideResolvesAtRequestTime
    TestModelRegistry.testBaseUrlOverrideDoesNotAffectOtherProviders
    TestModelRegistry.testMixBaseUrlOverrideAndCustomModels
    TestModelRegistry.testRefreshPicksUpBaseUrlChanges
    TestModelRegistry.testBuiltInProviderCustomModelInheritsApiAndBaseUrl
    TestModelRegistry.testNonBuiltInProviderRequiresBaseUrl
    TestModelRegistry.testCustomModelSameIdReplacesBuiltIn
    TestModelRegistry.testProviderCompatAppliesToCustomModels
    TestModelRegistry.testModelCompatOverridesProviderCompat
    TestModelRegistry.testModelBaseUrlOverridesProviderBaseUrl
    TestModelRegistry.testRemovingCustomModelsKeepsBuiltIn
    TestModelRegistry.testModelOverrideAppliesToSingleBuiltIn
    TestModelRegistry.testModelOverrideCompatOpenRouterRouting
    TestModelRegistry.testModelOverrideForNonExistentIdIgnored
    TestModelRegistry.testModelOverridePartialCost
    TestModelRegistry.testModelOverrideRequestTimeHeaders
    TestModelRegistry.testRemovingModelOverrideRestoresBuiltIn
    TestModelRegistry.testGetProviderDisplayNameFallbacks
    TestModelRegistry.testStoredEnvPropagatesToRequestAuth
    TestModelRegistry.testRegisterProviderUppercaseLiteral
    TestModelRegistry.testFailedRegisterProviderKeepsExisting
    TestModelRegistry.testDynamicBaseUrlOverridePersistsAcrossRefresh
    TestModelRegistry.testDynamicModelsReplacementPersistsAcrossRefresh
    TestModelRegistry.testDynamicHeadersOverridePersistsAcrossRefresh
    TestModelRegistry.testApiKeyCommandExecutes
    TestModelRegistry.testApiKeyCommandTrimsWhitespace
    TestModelRegistry.testApiKeyCommandFailureReturnsNone
    TestModelRegistry.testApiKeyEnvVar
    TestModelRegistry.testApiKeyBracedEnv
    TestModelRegistry.testApiKeyInterpolation
    TestModelRegistry.testApiKeyDollarDollarLiteral
    TestModelRegistry.testApiKeyLiteralUsedDirectly
    TestModelRegistry.testApiKeyCommandPerLookup
    TestModelRegistry.testAuthStatusEnvVar
    TestModelRegistry.testAuthStatusLiteralKey
    TestModelRegistry.testAuthStatusCommandNoExecution
    TestModelRegistry.testMissingEnvKeyUnconfiguredAndFiltered
    TestModelRegistry.testGetAvailableDoesNotExecuteCommands
    TestModelRegistry.testGetApiKeyAndHeadersAuthHeaderPerRequest
    TestModelRegistry.testGetApiKeyAndHeadersErrorForFailedAuthHeader
    ...
    TestTrustManager.testTrustStoreGetSet
    TestTrustManager.testTrustStoreSetMany
    TestTrustManager.testTrustStoreNearestAncestor
    TestTrustManager.testTrustOptions
    TestTrustManager.testHasTrustRequiringResources
    TestProjectTrust.testResolveProjectTrustedOverride
    TestProjectTrust.testResolveProjectTrustedNoResources
    TestProjectTrust.testResolveProjectTrustedSavedDecision
    TestProjectTrust.testResolveProjectTrustedDefaultAlways
    TestProjectTrust.testResolveProjectTrustedDefaultNever
    TestProjectTrust.testResolveProjectTrustedAskNoUI
