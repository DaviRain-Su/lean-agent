...
| Static model catalog | `LeanAgent.Models` | partial | Starter catalog covers DeepSeek, the current Pi OpenAI Responses model set, Azure OpenAI Responses current Pi model set, OpenRouter, Groq, xAI, Cerebras, Together, Fireworks representative OpenAI-compatible models, selected Anthropic Claude models, the current Pi Google Generative AI/Gemini model set, the current Pi Google Vertex Gemini model set, and the current Pi Mistral model set. Generated full catalog and dynamic refresh are missing. |
| Provider/model collection | `LeanAgent.Models`, `LeanAgent.AI.Providers.*` | partial | Runtime provider collection now supports registration, lookup, refresh hooks, auth application, auth-driven base URL/header overrides, simple stream/complete dispatch, explicit provider factories for the starter OpenAI-compatible provider family, OpenAI Responses provider/catalog dispatch, Anthropic Messages buffered SSE dispatch, Google Generative AI buffered SSE dispatch, Google Vertex buffered SSE dispatch, Mistral Conversations buffered SSE dispatch, Azure OpenAI Responses provider/catalog dispatch, generic OpenAI Responses dispatch, Azure OpenAI Responses stream dispatch, and Cloudflare Workers AI / AI Gateway provider factories for currently implemented OpenAI-compatible/Responses protocols. OpenAI-compatible, Responses, Anthropic, Google, Vertex, Mistral, and Azure `streamSimple` paths use streaming request formats but still buffer the HTTP response before returning events. Full generated catalog, dynamic providers, legacy CLI dispatch for non-Chat-Completions providers, Anthropic OAuth provider wiring, Anthropic-backed Gateway models, Vertex ADC token acquisition, and callback-style live streaming are missing. |
...
- OpenAI Codex Responses. Status: missing; OAuth and WebSocket/cached transport remain.
