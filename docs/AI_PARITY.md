...
- Anthropic Messages. Status: partial; payload/header/parser/provider dispatch, catalog-level provider headers, session-affinity compat wiring, prompt-cache cache-control payload markers, Anthropic-compatible Kimi Coding/MiniMax/MiniMax CN/Vercel factories, mixed OpenCode/OpenCode Go Anthropic-routed catalog entries, adaptive-thinking compat metadata, temperature suppression for thinking/unsupported models, and buffered SSE event reconstruction exist. OAuth/beta/live callback behavior remains.
