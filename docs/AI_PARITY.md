...
| Anthropic/Google/Mistral/Bedrock/Azure/Codex tests | provider protocol tests | partial | Anthropic Messages payload/header/response-parser/SSE parser/local streaming/provider catalog tests, Google Generative AI payload/response-parser/SSE parser/local streaming/provider catalog tests, Google Vertex URL/header/auth/local streaming/provider catalog tests, Azure OpenAI Responses base URL, deployment mapping, payload, local streaming transport, and compat built-in dispatch are covered. Anthropic OAuth/live callback tests, Vertex ADC token acquisition tests, and Mistral/Bedrock/Codex provider protocol tests are still missing. |
