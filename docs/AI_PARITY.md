...
| `src/legacy-api-aliases.ts` | `LeanAgent.AI.Compat.Aliases` | partial | Fixed-API legacy aliases exist for Anthropic, Bedrock Converse Stream, Azure OpenAI Responses, Google, Google Vertex, Mistral, OpenAI Codex Responses, OpenAI Completions, and OpenAI Responses. The Anthropic, Azure OpenAI Responses, OpenAI Responses, OpenAI Codex Responses, Google, and Mistral aliases now accept provider-specific options; the remaining aliases still dispatch through the compat registry with simple stream options, so typed full-stream option parity is incomplete. |
...
| `api/google-vertex.ts` | `LeanAgent.AI.Api.GoogleVertex` | partial | Vertex direct REST `generateContent`/`streamGenerateContent?alt=sse` URL construction, project/location resolution, `{location}` base URL replacement, custom versioned collection base URLs, API-key headers, ADC placeholder marker handling, payload construction via Google shared conversion, buffered SSE parsing via the Gemini parser, provider dispatch, and local HTTP tests exist. Automatic ADC OAuth token acquisition/refresh, Google SDK resource-scope behavior, abort semantics, and full Vertex option parity remain missing. |
...
- Google Generative AI and Google Vertex. Status: partial; Google Generative AI core, typed legacy alias dispatch, and Google Vertex direct REST/API-key/header-owned auth paths are implemented, Vertex ADC token acquisition remains missing.
