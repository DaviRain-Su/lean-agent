...
| `utils/oauth/device-code.ts` | `LeanAgent.AI.OAuth` | partial | Device-code polling supports `pending`, `slow_down`, `failed`, `complete`, default/minimum polling intervals, slow-down interval growth, deadline-capped sleeps, timeout messages, and injectable cancellation/clock/sleep for tests. AbortSignal-compatible mid-sleep cancellation is not modeled. |
