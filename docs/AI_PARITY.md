...
| --- | --- | --- | --- |
| `models-runtime.test.ts`, `providers.test.ts`, `supports-xhigh.test.ts`, `xhigh.test.ts` | model catalog and thinking tests | partial | Protects provider/model registry, explicit OpenAI-compatible provider factories, OpenAI Responses catalog metadata, cost calculation, thinking-level map helpers, auth-applied base URL overrides, and header-only auth dispatch. |
...
- Add provider factories for DeepSeek, OpenAI, OpenRouter, Groq, xAI, Cerebras, Together, Fireworks, Ant Ling, Hugging Face, Moonshot AI/CN, NVIDIA, Xiaomi, Xiaomi Token Plan AMS/CN/SGP, Z.AI, and Z.AI Coding CN where they share OpenAI-compatible protocol. Status: partial; provider modules and checked-in Pi catalog slices exist for these providers, and Anthropic-backed Kimi Coding/MiniMax/MiniMax CN/Vercel AI Gateway now exist under the Anthropic Messages protocol. Generated refresh tooling, per-model custom headers, and the remaining mixed provider factories OpenCode and OpenCode Go are missing.
...
- Anthropic Messages. Status: partial; payload/header/parser/provider dispatch, catalog-level provider headers, Anthropic-compatible Kimi Coding/MiniMax/MiniMax CN/Vercel factories, adaptive-thinking compat metadata, temperature suppression for thinking/unsupported models, and buffered SSE event reconstruction exist. OAuth/beta/live callback behavior and mixed-API OpenCode providers remain.
