...
| `src/legacy-api-aliases.ts` | `LeanAgent.AI.Compat.Aliases` | partial | Fixed-API legacy aliases exist for Anthropic, Bedrock Converse Stream, Azure OpenAI Responses, Google, Google Vertex, Mistral, OpenAI Codex Responses, OpenAI Completions, and OpenAI Responses. The Anthropic, Azure OpenAI Responses, OpenAI Responses, OpenAI Codex Responses, and Mistral aliases now accept provider-specific options; the remaining aliases still dispatch through the compat registry with simple stream options, so typed full-stream option parity is incomplete. |
...
- Google Generative AI and Google Vertex. Status: partial; Google Generative AI core and Google Vertex direct REST/API-key/header-owned auth paths are implemented, Vertex ADC token acquisition remains missing.
