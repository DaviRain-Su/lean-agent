...
| `api/anthropic-messages.ts` | `LeanAgent.AI.Api.AnthropicMessages` | partial | Anthropic Messages request payload/header construction, API-key/header-owned/OAuth-token auth header shapes, session-affinity header compat wiring, prompt cache `cache_control` payload markers for system prompts, last user blocks, and last tool definitions with long-retention TTL support, eager tool input streaming compat plus legacy fine-grained beta header fallback, tool-level cache-control suppression, empty thinking-signature replay opt-in, cross-provider message transform reuse, system/tools/tool_result/tool_use conversion, non-streaming HTTP completion, buffered SSE request/response parsing, text/thinking/tool delta event reconstruction, usage mapping, stop-reason mapping, terminal `message_stop` validation, provider dispatch, and tests exist. Native callback-style live streaming, full beta header matrix, OAuth login/refresh provider wiring, adaptive thinking effort metadata, and remaining generated compat metadata are missing. |
