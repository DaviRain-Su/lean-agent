...
| Compat/global API registry | `LeanAgent.AI.Compat`, `LeanAgent.AI.Compat.Aliases` | partial | Global API provider registry, built-in OpenAI-compatible/OpenAI Responses/OpenAI Codex Responses/Azure OpenAI Responses/Google/Google Vertex/Mistral registrations, source-id unregister, reset, `streamSimple`/`completeSimple` dispatch, fixed-API legacy aliases, Anthropic/Azure OpenAI Responses/OpenAI Completions/OpenAI Responses/OpenAI Codex Responses/Google/Google Vertex/Mistral typed legacy stream aliases, API mismatch guard, and Pi mapped provider env API-key injection exist. Full legacy static catalog exports, image entrypoints, typed full-stream options for the remaining legacy aliases, and several non-OpenAI builtins are still missing. |
...
| `src/compat.ts` | `LeanAgent.AI.Compat` | partial | Global registry, simple dispatch, and legacy alias support exist, including built-in Bedrock Converse Stream registration. Full legacy surface, static catalog passthroughs, image exports, faux compat registration, and typed full-stream options are still missing. |
...
| `src/legacy-api-aliases.ts` | `LeanAgent.AI.Compat.Aliases` | partial | Fixed-API legacy aliases exist for Anthropic, Bedrock Converse Stream, Azure OpenAI Responses, Google, Google Vertex, Mistral, OpenAI Codex Responses, OpenAI Completions, and OpenAI Responses. The Anthropic, Azure OpenAI Responses, OpenAI Completions, OpenAI Responses, OpenAI Codex Responses, Google, Google Vertex, and Mistral aliases now accept provider-specific options; the remaining aliases still dispatch through the compat registry with simple stream options, so typed full-stream option parity is incomplete. |
...
| `api/bedrock-converse-stream.ts` | `LeanAgent.AI.Api.BedrockConverseStream` | partial | ConverseStream command payload construction, text/image/tool/toolResult/thinking conversion, cache-point/reasoning/tool-choice/metadata helpers, stop-reason/endpoint/header helpers, payload hooks, provider dispatch, and explicit SigV4/event-stream transport boundary tests exist. Real AWS SDK/default credential chain, SigV4 signing, Smithy proxy middleware, abort semantics, response metadata hooks, and streaming event parsing/runtime are still missing. |
