...
- OpenAI Codex Responses. Status: partial; provider factory, current Pi model catalog, stored OAuth access-token auth, Codex SSE fallback request/header semantics, shared Responses parsing, compat dispatch, and local tests exist; WebSocket/cached transport plus OAuth login/refresh remain missing.
