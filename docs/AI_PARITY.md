...
| `api/anthropic-messages.ts` | `LeanAgent.AI.Api.AnthropicMessages` | partial | Anthropic Messages request payload/header construction, API-key/header-owned/OAuth-token auth header shapes, session-affinity header compat wiring, prompt cache `cache_control` payload markers for system prompts, last user blocks, and last tool definitions with long-retention TTL support, eager tool input streaming compat plus legacy fine-grained beta header fallback, interleaved-thinking beta header defaults with adaptive-thinking suppression, tool-level cache-control suppression, empty thinking-signature replay opt-in, cross-provider message transform reuse, system/tools/tool_result/tool_use conversion, non-streaming HTTP completion, buffered SSE request/response parsing, text/thinking/tool delta event reconstruction, usage mapping, stop-reason mapping, terminal `message_stop` validation, provider dispatch, and tests exist. Native callback-style live streaming, OAuth login/refresh provider wiring, adaptive thinking effort metadata, and remaining generated compat metadata are missing. |
...
| `api/mistral-conversations.ts` | `LeanAgent.AI.Api.MistralConversations` | partial | Mistral direct REST `/v1/chat/completions` payload/header construction, 9-character tool-call id normalization, system/user/assistant/tool conversion, tool schema serialization, prompt cache `prompt_cache_key` and `x-affinity`, reasoning `prompt_mode`/`reasoning_effort` mapping, non-streaming response parsing, buffered SSE text/thinking/tool delta reconstruction, usage cache-read mapping, stop-reason mapping, provider dispatch, and local HTTP tests exist. Native SDK abort semantics, callback-style live streaming, exact SDK request-option behavior, and full compat option parity remain missing. |
...
- Mistral Conversations. Status: partial; direct REST payload/header/parser/provider dispatch and buffered SSE event reconstruction exist, SDK abort/live callback behavior remains.
