...
| Images | `LeanAgent.AI.Types`, `LeanAgent.AI.Images`, `LeanAgent.AI.Images.Registry`, `LeanAgent.AI.Api.OpenRouterImages`, `LeanAgent.AI.Images.Models` | partial | Image content/model/option/result types, global image API registry, API mismatch guard, source unregister/reset with built-in replay, `generateImages` dispatch, OpenRouter Images runtime, and the current Pi OpenRouter image model catalog exist. Pi-style image provider collection/auth factory remains missing. |
...
| `images-models.ts` | `LeanAgent.AI.Images.Models` | partial | Runtime image model type and lookup helpers exist. Pi-style image provider collection remains missing. |
...
| `images*.test.ts`, `openrouter-images.test.ts` | image tests | partial | Generic image API registry, built-in replay, full current image catalog lookup/count, OpenRouter Images payload construction, local HTTP dispatch, headers, response hooks, output parsing, usage/cost parsing, and missing-key errors are covered. Live provider matrix tests are still missing. |
