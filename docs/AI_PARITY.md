...
| --- | --- | --- | --- |
| `src/index.ts` | `LeanAgent.AI` or `LeanAgent.lean` exports | partial | Lean root exports core AI modules plus image dispatch modules, but not the full Pi AI public surface. |
...
| `src/images-models.ts` | `LeanAgent.AI.Images`, `LeanAgent.AI.Images.Models` | partial | Runtime image model type, lookup functions, mutable image provider collection, provider CRUD, model refresh, auth application, and error-as-result `generateImages` exist. OAuth credential behavior and image-specific file-backed auth tests remain missing. |
...
| `auth/credential-store.ts` | `LeanAgent.AI.Auth` | partial | In-memory and JSON file-backed API-key credential stores exist. OAuth serialization/persistence awaits OAuth credential types. |
...
| `auth/resolve.ts` | `LeanAgent.AI.Auth` | partial | API-key provider auth resolution with request overrides and optional model base URL context exists. OAuth refresh/error codes are missing. |
...
| `images-models.ts` | `LeanAgent.AI.Images`, `LeanAgent.AI.Images.Models` | partial | Runtime image model type, lookup helpers, image provider collection, provider factory, refresh, auth merge, and error-as-result dispatch exist. OAuth credential behavior and image-specific file-backed auth tests remain missing. |
...
- Env API key auth, in-memory and file-backed API-key credential stores, auth resolution. Status: partial; OAuth credentials are missing.
