...
| `src/legacy-api-aliases.ts` | `LeanAgent.AI.Compat.Aliases` | partial | Fixed-API legacy aliases exist for Anthropic, Bedrock Converse Stream, Azure OpenAI Responses, Google, Google Vertex, Mistral, OpenAI Codex Responses, OpenAI Completions, and OpenAI Responses. The Mistral alias now accepts provider-specific `MistralOptions`; the remaining aliases still dispatch through the compat registry with simple stream options, so typed full-stream option parity is incomplete. |
...
- OpenAI Responses. Status: partial; provider factory, current Pi model catalog, auth, generic/local streaming dispatch, and tests exist; native callback streaming, generated catalog refresh, and legacy CLI Responses selection remain.
