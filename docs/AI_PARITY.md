...
| OAuth/auth store | `LeanAgent.AI.Auth`, `LeanAgent.AI.OAuth`, `LeanAgent.AI.OAuth.Page`, `LeanAgent.AI.EnvApiKeys`, `LeanAgent.AI.Providers.CloudflareAuth` | partial | Env API-key auth, Pi provider env-key map, auth context with injectable wall-clock milliseconds, in-memory and file-backed API-key/OAuth credentials, provider auth resolution, OAuth refresh/toAuth resolution under credential-store modify, OAuth provider registry, OAuth provider info list, high-level refresh/API-key helpers, device-code polling, callback success/error page rendering, model-aware base URL auth resolution, and Cloudflare Workers AI / AI Gateway auth helpers exist. OAuth login providers, callback-server flows, lazy OAuth wrappers, file locks, and typed ModelsError codes are missing. |
