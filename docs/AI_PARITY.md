...
| `api/openai-responses.ts` | `LeanAgent.AI.Api.OpenAIResponses` | partial | Responses request payload construction, prompt-cache/session affinity fields, cache-affinity/custom headers, GitHub Copilot dynamic headers, non-streaming HTTP completion, provider error handling, usage parsing, service-tier cost multipliers, reasoning/message/function-call output parsing, streaming SSE event parsing, terminal-event enforcement, tool-call argument delta handling, and HTTP streaming wrapper exist. SDK-equivalent request hooks and provider dispatch integration are still missing. |
