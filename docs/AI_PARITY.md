...
| `api/openai-prompt-cache.ts` | `LeanAgent.AI.Api.OpenAIPromptCache` | partial | Prompt cache key clamping and Chat Completions payload fields exist, including `cacheRetention=none`, `long`, and `PI_CACHE_RETENTION=long`; Chat Completions session-affinity headers are wired through `sendSessionAffinityHeaders`; provider-specific long-cache suppression is honored for OpenAI-compatible models. Anthropic-compatible session affinity and broader generated compat metadata remain incomplete. |
