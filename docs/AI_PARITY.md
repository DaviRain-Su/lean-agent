...
| --- | --- | --- | --- |
| HTTP transport | `LeanAgent.Http` | partial | Native libcurl JSON POST, custom request headers, and parsed response headers exist. Streaming callbacks, retry hooks, and proxy helpers are missing. |
...
| `retry.test.ts`, `diagnostics.test.ts`, `estimate.test.ts`, `overflow.test.ts`, `validation.test.ts`, `unicode-surrogate.test.ts` | util tests | partial | Retry classifier/policy, diagnostics extraction/round-trip, estimate utilities, JSON repair/streaming fallback, overflow detection, and OpenAI transient HTTP retry are covered. Validation and unicode surrogate tests are missing. |
