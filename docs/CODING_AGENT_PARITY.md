...
| `src/utils/child-process.ts` | `LeanAgent.CodingAgent.Utils.ChildProcess` | partial | `ChildProcessResult` + `spawnProcess` (IO.Process.spawn piped stdout/stderr) + `spawnProcessSync` (sync spawn with timeout poll, returns `ChildProcessResult`) + `waitForChildProcess` (wait + EXIT_STDIO_GRACE_MS post-exit sleep for stdio drain); `cross-spawn` not needed (Lean IO.Process handles PATH). Event-driven stdio idle re-arm is Node-stream-specific — approximated with grace sleep. |
...
| `src/utils/clipboard.ts` | `LeanAgent.CodingAgent.Utils.Clipboard` | partial | `copyToClipboard` (platform tools: pbcopy/clip/wl-copy/xclip/xsel/termux-clipboard-set via IO.Process.spawn piped stdin) + OSC 52 fallback (`emitOsc52` base64 + escape seq, `MAX_OSC52_ENCODED_LENGTH`) + `isRemoteSession` (SSH_CONNECTION/SSH_CLIENT/MOSH_CONNECTION) + `isWaylandSession` (inlined from clipboard-image.ts: WAYLAND_DISPLAY/XDG_SESSION_TYPE) + local `base64Encode`. Native clipboard addon (`clipboard-native.ts`) treated as unavailable (Node native module). |
...
| `src/utils/fs-watch.ts` | `LeanAgent.CodingAgent.Utils.FsWatch` | partial | `FsWatcher` interface (close/isOpen) + `closeWatcher` (best-effort) + `watchWithErrorHandler` (returns none + calls onError — no native fs.watch in Lean) + `FS_WATCH_RETRY_DELAY_MS` constant. Real watcher impl deferred (needs external process like fswatch/watchman). |
...
| `src/utils/image-convert.ts` | `LeanAgent.CodingAgent.Utils.ImageConvert` | partial | `convertImageBytesToPng` (returns none — Photon WASM Exclusion) + `convertToPng` (PNG fast path returns as-is; else decode→convert→re-encode, returns none without Photon) + local `base64Encode`/`base64Decode`. Photon-dependent conversion is Exclusion List §7-adjacent. |
| `src/utils/image-process.ts` | `LeanAgent.CodingAgent.Utils.ImageProcess` | partial | `ProcessImageOptions`/`ProcessImageResult` (ok/error) + `baseMimeType`/`normalizeSupportedImageMimeType`/`normalizeImage`/`conversionHint`/`processImage` (pure MIME normalization ported; resize returns error — Photon Exclusion). **Lean-side addition**: `getImageDimensions` parses PNG IHDR / JPEG SOF0/SOF2 / GIF logical-screen-descriptor / WebP VP8/VP8L/VP8X headers (not in Pi; Pi uses photon.get_width/get_height). |
...
| `src/utils/photon.ts` | `LeanAgent.CodingAgent.Utils.Photon` | partial | `loadPhoton` (always returns none — Photon Rust/WASM not available in Lean; Exclusion List §7-adjacent) + `PhotonImage` stub type. Signature preserved so dependent modules compile. |
...
| `src/utils/shell.ts` | `LeanAgent.CodingAgent.Utils.Shell` | implemented | `ShellConfig`+`CommandTransport`; `isLegacyWslBashPath` (drive:\windows\{system32,sysnative}\bash.exe regex) + `getBashShellConfig` (WSL `-s`/stdin, else `-c`); `findBashOnPath` (`where bash.exe`+existsSync verify on Windows, `which bash` trust on Unix) + `getShellConfig` (custom→Windows Git Bash→PATH; Unix `/bin/bash`→PATH→`sh` fallback; throws on Windows miss) over injectable `ShellDeps` (`defaultShellDeps` wires real IO); `getShellEnv` (case-insensitive PATH key + binDir augment, returns `(key,value)` pair; Pi spreads env) + `findPathKey`; `killProcessTree` (win `taskkill /F /T`, unix `kill -9 -<pid>` process-group + single-pid fallback) + `track`/`untrack`/`killTrackedDetachedChildren` (global `IO.Ref` set) + `sanitizeBinaryOutput` re-export from Harness.Truncate (`TestShell.*`). |
...
| `src/utils/windows-self-update.ts` | `LeanAgent.CodingAgent.Utils.WindowsSelfUpdate` | partial | `QUARANTINE_DIR_NAME` + `checkForSelfUpdate`/`performSelfUpdate`/`cleanupWindowsSelfUpdateQuarantine`/`quarantineWindowsNativeDependencies` (all no-ops — require Node process.report V8 internal + Bun executable replace; Exclusion List §7-adjacent). |
