...
| Compaction / branch | `Agent.Harness.Compaction` | partial | Full offline pipeline: CompactionSettings (enabled/reserveTokens/keepRecentTokens/maxMessages/maxTokens/compactionThreshold), estimateTokenCount (chars/4 heuristic), shouldCompact, selectCompactionWindow, buildCompactionSummary, prepareCompactionAuto (returns Option CompactionResult), compact (auto), compactSession, compactWithSummary (legacy explicit-summary path); backward-compatible prepareCompaction returns PrepareCompactionResult for CodingAgent; branch summary helpers. Not full Pi compaction: no live-LLM generateSummary, no CompactionPreparation with split-turn/turnPrefix, no file-ops extraction. |
...
| AgentHarness façade | `Agent.Harness.AgentHarness` | partial | prompt/steer/followUp/nextTurn/appendMessage/compact/setModel/setThinkingLevel + createUserMessage/createFailureMessage/cloneStreamOptions/applyStreamOptionsPatch/findDuplicateNames/subscribe/unsubscribe/navigateTree/getResource?/setResource/getPendingWrites/flushPendingWrites/HarnessStreamOptions + AgentHarnessRef (IO.Ref-backed) (`testAgentHarnessAppendCompactSetters`); still << Pi agent-harness.ts (~1k LOC) — missing live-LLM summarization, skill/promptFromTemplate, setTools/setActiveTools, abort phase events. |
...
| Pi source | Lean target | Status | Notes |
| --- | --- | --- | --- |
| `harness/agent-harness.ts` | `Harness.AgentHarness` + `Harness.AgentHarnessRef` | partial | prompt/steer/followUp/nextTurn/appendMessage/compact/setModel/setThinkingLevel + createUserMessage/createFailureMessage/cloneStreamOptions/applyStreamOptionsPatch/findDuplicateNames/subscribe/unsubscribe/navigateTree/getResource?/setResource/getPendingWrites/flushPendingWrites/HarnessStreamOptions + AgentHarnessRef (IO.Ref-backed subscribe/unsubscribe/emit/getResource?/setResource/getPendingWrites/flushPendingWrites); compactAuto (auto pipeline) + compact (explicit summary); still missing skill/promptFromTemplate/setTools/setActiveTools/abort phase events/live-LLM hooks. |
...
| `harness/messages.ts` | `Harness.Messages` + `Harness.Compaction` custom helpers | partial | `convertToLlm` (AgentMessage→Message array, handles compactionSummary/branchSummary/bashExecution/custom→user mapping); COMPACTION_SUMMARY_PREFIX/SUFFIX, BRANCH_SUMMARY_PREFIX/SUFFIX; bashExecutionToText; createCompactionSummaryMessage/createBranchSummaryMessage/createCustomMessage. Not full: Pi message type matrix (excludeFromContext on bashExecution, details on CustomMessage). |
...
| `harness/compaction/compaction.ts` | `Harness.Compaction` | partial | Full offline pipeline: CompactionSettings, estimateTokenCount (chars/4), shouldCompact, selectCompactionWindow, buildCompactionSummary, prepareCompactionAuto, compact (auto), compactSession, compactWithSummary (legacy); backward-compatible prepareCompaction returns PrepareCompactionResult. Missing: live-LLM generateSummary, CompactionPreparation with split-turn/turnPrefix, file-ops extraction, calculateContextTokens from usage. |
